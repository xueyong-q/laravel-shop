...
class PaymentController extends Controller
{
    ...
    public function payByInstallment(Order $order, Request $request)
    {
        ...
        // 计算每一期的本金
        $base = (string) BigDecimal::of($order->total_amount)->dividedBy($count, 2, RoundingMode::DOWN);

        // 计算每一期的手续费
        $fee = (string) BigDecimal::of($base)->multipliedBy($installment->fee_rate)->dividedBy(100, 2, RoundingMode::DOWN);

        // 根据用户选择的还款期数，创建对应数量的还款计划
        for ($i = 0; $i < $count; $i++) {
            // 最后一期的本金需要用总本金减去前面几期的本金
            if ($i === $count - 1) {
                $base = (string) BigDecimal::of($order->total_amount)->minus(BigDecimal::of($base)->multipliedBy($count - 1));
            }
